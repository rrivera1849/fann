...
#define fann_rand_uniform() ((float) rand () / (float) RAND_MAX)
#define fann_rand_normal_dist() (sqrtf (-2 * log (fann_rand_uniform ())) * cos (2 * M_PI * fann_rand_uniform ()))
#define fann_rand_normal_dist_fast() ({ \
  float x1, x2, w, y1, y2; \
  do { \
    x1 = 2.0 * fann_rand_uniform () - 1.0; \
    x2 = 2.0 * fann_rand_uniform () - 1.0; \
    w = x1 * x1 + x2 * x2; \
  } while (w >= 1.0); \
  w = sqrtf((-2.0 * log (w)) / w); \
  y1 = x1 * w; \
