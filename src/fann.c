		...
		for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
		{
			...
			switch (activation_function)
			{
				...
				case FANN_LINEAR_PIECE_SYMMETRIC:
					...
					break;
        case FANN_RELU:
          neuron_it->value = (fann_type) (neuron_sum > 0.0 ? neuron_sum : 0);
          break;
				case FANN_ELLIOT:
...
FANN_EXTERNAL void FANN_API fann_randomize_weights_normalize_variance(struct fann *ann)
{ 
  struct fann_layer *current_layer = ann->first_layer;
  struct fann_layer *last_layer = ann->last_layer;
  struct fann_neuron *current_neuron = NULL;
  struct fann_neuron *last_neuron = NULL;

  int neurons_in, neurons_out;
  neurons_in = neurons_out = 0;

  int current_con, bias_con;
  current_con = bias_con = 0;

  for (; current_layer != last_layer; current_layer++)
  {
    // Loop through every layer and do the following operation
    // to initialize each weight:
    // W = fann_rand_uniform () / fann_in
    // Where fann_in is the total number of neurons from the last layer
    current_neuron = current_layer->first_neuron;
    last_neuron = current_layer->last_neuron - 1;

    neurons_in = neurons_out;
    neurons_out = last_neuron - current_neuron;

    // We're in the first layer, skip
    if (neurons_in == 0)
      continue;

    for (; current_neuron != last_neuron; current_neuron++)
    {
      bias_con = current_neuron->last_con - 1;
      for (current_con = current_neuron->first_con; 
           current_con != bias_con; current_con++)
      {
        ann->weights[current_con] = 
          (fann_type) (fann_rand_normal_dist_fast () / sqrtf ((float) neurons_in));
      }
      
      // Bias neuron is initialized to 0
      ann->weights[bias_con] = (fann_type) 0;
    }
  }
}
